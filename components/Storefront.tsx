...
export default function Storefront() {
  ...
  const visibleFilters = useMemo(() => {
    const chunkSize = 4;
    const start = filterOffset % visualFilters.length;
    return Array.from({ length: chunkSize }, (_, index) => visualFilters[(start + index) % visualFilters.length]);
  }, [filterOffset]);
  ...
  const handleOrder = (product: Product) => {
    const message = makeWhatsAppMessage([{ product, quantity: 1, color: 'Dorado', details: '' }]);
    window.open(`https://wa.me/541131196403?text=${encodeURIComponent(message)}`, '_blank');
  };

  const handleCheckout = () => {
    if (cart.length === 0) return;
    const message = makeWhatsAppMessage(cart);
    window.open(`https://wa.me/541131196403?text=${encodeURIComponent(message)}`, '_blank');
  };
  ...
  return (
    <main>
      ...
      <div className="promo-banner">
        🎉 ¡Comprando <strong>10 unidades o más</strong> de un mismo artículo, accedés automáticamente al <strong>Precio Mayorista</strong> de súper descuento!
      </div>

      <section className="workflow-section">
        <h2>💌 Invitaciones Digitales Personalizadas</h2>
        <p style={{ maxWidth: 850, margin: '0 auto 30px auto', color: 'var(--silver-metal)', fontSize: '1rem', lineHeight: 1.7 }}>
      ...
      <div className="filter-bar">
        <button className="carousel-nav" onClick={() => setFilterOffset((value) => value - 1)} aria-label="Mostrar filtros anteriores">
          ‹
        </button>
        <div className="filter-pills">
          {visibleFilters.map((filter) => (
            <button
              key={filter.id}
              className={`filter-pill ${activeCategory === filter.id ? 'active' : ''}`}
        ...
        <button className="carousel-nav" onClick={() => setFilterOffset((value) => value + 1)} aria-label="Mostrar filtros siguientes">
          ›
        </button>
        <div className="view-toggle" role="tablist" aria-label="Cambiar vista de productos">
          <button className={`view-toggle-btn ${viewMode === 'gallery' ? 'active' : ''}`} onClick={() => setViewMode('gallery')}>
            🖼 Galería
          </button>
      ...
      {cart.length > 0 ? (
        <button className="btn-pedir" onClick={handleCheckout} style={{ position: 'fixed', right: 20, bottom: 20, zIndex: 2001, width: 'auto', padding: '12px 18px', borderRadius: 999 }}>
